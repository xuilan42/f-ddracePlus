...
void CServer::GenerateServerInfo(CPacker *pPacker, int Token, int Socket)
{
	...
	// gametype
	pPacker->AddString(Config()->m_SvGametype, 16);
...
void CServer::SendServerInfoSevendown(const NETADDR *pAddr, int Token, int Socket)
{
	...
	p.AddString(Config()->m_SvGametype, 16);
...
const char *CServer::GetGameTypeServerInfo()
{
	...
	// Use sv_gametype config instead of GameServer()->GameType()
	str_copy(aBuf, Config()->m_SvGametype, sizeof(aBuf));
	if (IsBrowserScoreFix())
	{
		...
		const char *pGameType = Config()->m_SvGametype;
...
int CServer::Run()
{
	...
	// F-DDrace: Process server console input
	GameServer()->OnPreShutdown();
...
void CServer::ConchainEconConfigOnly(IConsole::IResult *pResult, void *pUserData, IConsole::FCommandCallback pfnCallback, void *pCallbackUserData)
{
	// Allow from server console/config files, but not from in-game rcon.
	if(pResult->m_ClientID >= 0)
	{
		CServer *pThis = static_cast<CServer *>(pUserData);
		pThis->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "econ", "This setting can only be changed from server config files.");
		return;
	}
	pfnCallback(pResult, pCallbackUserData);
}
...
void CServer::RegisterCommands()
{
	...
	// Econ settings should not be changeable from in-game rcon.
	Console()->Chain("ec_port", ConchainEconConfigOnly, this);
	Console()->Chain("ec_bantime", ConchainEconConfigOnly, this);
	Console()->Chain("ec_bindaddr", ConchainEconConfigOnly, this);
	Console()->Chain("ec_password", ConchainEconConfigOnly, this);
	Console()->Chain("ec_auth_timeout", ConchainEconConfigOnly, this);
	Console()->Chain("ec_output_level", ConchainEconConfigOnly, this);
