...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, bool NoDamage, int ActivatedTeam, Mask128 Mask)
{
	...
	for (int i = 0; i < Num; i++)
	{
		...
		if ((GetPlayerChar(Owner) ? !(GetPlayerChar(Owner)->m_Hit & CCharacter::DISABLE_HIT_GRENADE) : Config()->m_SvHit || NoDamage) || (pChr && Owner == pChr->GetPlayer()->GetCID()))
		{
			...
			if (IsCharacter)
			{
				if (pChr->GetPlayer()->m_Invincible)
				{
					CreateDeath(pChr->GetPos(), pChr->GetPlayer()->GetCID(), pChr->TeamMask());
					continue;
				}
...
void CGameContext::PreInputClients(int ClientId, bool *pClients)
{
	...
	if(m_apPlayers[ClientId]->GetTeam() == TEAM_SPECTATORS || !m_apPlayers[ClientId]->GetCharacter() || m_apPlayers[ClientId]->m_Afk)
		return;
...
void CGameContext::OnClientEnter(int ClientID)
{
	...
	m_apPlayers[ClientID]->InitJoinCaptcha();
	m_apPlayers[ClientID]->SendJoinCaptcha();
	...
	SendChatTarget(ClientID, "F-DDrace+ Mod. Version: " GAME_VERSION ", by xuilan42");
	...
	if(Seconds != 0 && !Config()->m_SvCaptcha && m_apPlayers[ClientID]->m_JoinTick > m_NonEmptySince + 10 * Server()->TickSpeed() && Server()->GetDummy(ClientID) == -1)
	{
		char aBuf[128];
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		if(MsgID == NETMSGTYPE_CL_SAY)
		{
			...
			if(pPlayer->IsJoinCaptchaPending())
			{
				pPlayer->CheckJoinCaptchaAnswer(pMsg->m_pMessage);
				return;
			}

			// drop empty and autocreated spam messages (more than 32 characters per second)
			// F-DDrace: Check if chat is frozen for new players (anti-spam on join)
			if (pPlayer->m_ChatFrozenUntilTick > Server()->Tick() && pMsg->m_pMessage[0] != '/')
			{
				SendChatTarget(ClientID, "You cannot chat yet. Move around to unlock chat.");
				return;
			}
...
void CGameContext::ConCalcTune(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	const float Scale = pResult->GetFloat(0);
	if(Scale <= 0.0f)
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "tuning", "scale must be > 0");
		return;
	}

	const int TickSpeed = max(1, (int)roundf((1.0f / Scale) * 50.0f));
	const float TickScale = TickSpeed / 50.0f;
	auto Linear = [TickScale](float Value) { return Value / TickScale; };
	auto LinearInv = [TickScale](float Value) { return Value * TickScale; };
	auto Accel = [TickScale](float Value) { return Value / (TickScale * TickScale); };
	auto Friction = [TickScale](float Value) { return powf(Value, 1.0f / TickScale); };

	CTuningParams *pTuning = pSelf->Tuning();
	pTuning->m_GroundControlSpeed = Linear(10.0f);
	pTuning->m_GroundControlAccel = Accel(2.0f);
	pTuning->m_GroundFriction = Friction(0.5f);
	pTuning->m_GroundJumpImpulse = Linear(13.2f);
	pTuning->m_AirJumpImpulse = Linear(12.0f);
	pTuning->m_AirControlSpeed = Linear(5.0f);
	pTuning->m_AirControlAccel = Accel(1.5f);
	pTuning->m_AirFriction = Friction(0.95f);
	pTuning->m_HookFireSpeed = Linear(80.0f);
	pTuning->m_HookDragAccel = Accel(3.0f);
	pTuning->m_HookDragSpeed = Linear(15.0f);
	pTuning->m_Gravity = Accel(0.5f);
	pTuning->m_VelrampStart = Linear(550.0f);
	pTuning->m_VelrampRange = Linear(2000.0f);
	pTuning->m_GunSpeed = Linear(2200.0f);
	pTuning->m_GunLifetime = LinearInv(2.0f);
	pTuning->m_GrenadeSpeed = Linear(1000.0f);
	pTuning->m_GrenadeLifetime = LinearInv(2.0f);
	pTuning->m_LaserBounceDelay = LinearInv(150.0f);
	pTuning->m_JetpackStrength = Linear(400.0f);
	pTuning->m_ShotgunStrength = Linear(10.0f);
	pTuning->m_ExplosionStrength = Linear(6.0f);
	pTuning->m_HammerStrength = Linear(1.0f);
	pTuning->m_HookDuration = LinearInv(1.25f);
	pTuning->m_HammerFireDelay = LinearInv(125.0f);
	pTuning->m_GunFireDelay = LinearInv(125.0f);
	pTuning->m_ShotgunFireDelay = LinearInv(500.0f);
	pTuning->m_GrenadeFireDelay = LinearInv(500.0f);
	pTuning->m_LaserFireDelay = LinearInv(800.0f);
	pTuning->m_NinjaFireDelay = LinearInv(800.0f);
	pTuning->m_HammerHitFireDelay = LinearInv(320.0f);

	pSelf->SendTuningParams(-1);

	char aBuf[128];
	str_format(aBuf, sizeof(aBuf), "Applied calculated tunes with scale %.3f (tick_speed=%d)", Scale, TickSpeed);
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "tuning", aBuf);
}

void CGameContext::ConAdminTuneParam(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	const char *pParamName = pResult->GetString(0);
	float NewValue = pResult->NumArguments() == 2 ? pResult->GetFloat(1) : -1;
	...
	if(NewValue != -1 && pSelf->AdminTuning()->Set(pParamName, NewValue))
	{
		str_format(aBuf, sizeof(aBuf), "%s (admin) changed to %.2f", pParamName, NewValue);
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "tuning", aBuf);
	}
	else if (pSelf->AdminTuning()->Get(pParamName, &Value))
	{
		str_format(aBuf, sizeof(aBuf), "Value: %.2f", Value);
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "console", aBuf);
	}
...
void CGameContext::ConExec(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	...
	{
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "exec", "Usage: exec <file>");
		return;
	}

	if(!pSelf->Console()->ExecuteFile(pFilename, -1, true))
	{
		char aBuf[256];
		str_format(aBuf, sizeof(aBuf), "Could not execute file '%s'", pFilename);
		pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "exec", aBuf);
	}
}
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("sv_calc_tune", "f[scale]", CFGFLAG_SERVER|CFGFLAG_GAME, ConCalcTune, this, "Apply tune_calculator scaling to physics tunes", AUTHED_ADMIN);
	...
	Console()->Register("admin_tune", "s[tuning] ?i[value]", CFGFLAG_SERVER|CFGFLAG_GAME, ConAdminTuneParam, this, "Tune variable for admins only", AUTHED_ADMIN);
	Console()->Register("exec", "r[file]", CFGFLAG_SERVER, ConExec, this, "Execute commands from a config file", AUTHED_ADMIN);
...
void CGameContext::OnInit()
{
	...
	// F-DDrace: Initialize admin tuning (copy from normal tuning, will be modified by consolevar changes)
	m_AdminTuning = m_Tuning;
...
{
	char aFooter[128];
	str_format(aFooter, sizeof(aFooter), "F-DDrace+ is a mod by xuilan42\nF-DDrace+ Mod. Ver.: %s", GAME_VERSION);
...
int CGameContext::GetProjectileType(int Weapon) const
{
	...
	case WEAPON_BALL_GRENADE:
		return WEAPON_GRENADE;
	case WEAPON_BLACKHOLE_GRENADE:  // F-DDrace: Black hole is a type of grenade
		return WEAPON_GRENADE;
	}
	return Weapon;
}
