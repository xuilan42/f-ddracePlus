...
void CGameContext::ConKillPlayer(IConsole::IResult *pResult, void *pUserData)
{
	...
	{
		if (pSelf->m_apPlayers[Victim]->m_Invincible)
		{
			pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "console", "Target is invincible");
			return;
		}
...
void CGameContext::ModifyWeapons(IConsole::IResult* pResult, void* pUserData, int Weapon, bool Remove, bool AddRemoveCommand)
{
	...
	if (Weapon < 0)
	{
		...
		if (Weapon == -3)
		{
			pChr->m_NinjaSword = !Remove;
			pChr->ScrollNinja(!Remove, pResult->m_ClientID, true);
		}
	}
	else
	{
...
void CGameContext::ConNinjaSword(IConsole::IResult *pResult, void *pUserData)
{
	CGameContext *pSelf = (CGameContext *)pUserData;
	int Victim = pResult->NumArguments() ? pResult->GetVictim() : pResult->m_ClientID;
	CCharacter *pChr = pSelf->GetPlayerChar(Victim);
	...
	pChr->m_NinjaSword = !pChr->m_NinjaSword;
	if(pChr->m_NinjaSword)
		pChr->ScrollNinja(true, pResult->m_ClientID);

	char aBuf[96];
	str_format(aBuf, sizeof(aBuf), "Ninja sword mode for '%s': %s", pSelf->Server()->ClientName(Victim), pChr->m_NinjaSword ? "ON" : "OFF");
...
void CGameContext::ConInvincible(IConsole::IResult *pResult, void *pUserData)
{
	...
	pVictim->m_Invincible = !pVictim->m_Invincible;
	CCharacter *pChr = pSelf->GetPlayerChar(Victim);
	if(pChr)
	{
		if(pVictim->m_Invincible)
		{
			pChr->m_Super = true;
			pChr->UnFreeze();
			pChr->m_TeamBeforeSuper = pChr->Team();
			pChr->Teams()->SetCharacterTeam(Victim, TEAM_SUPER);
			pChr->InfiniteJumps(true, -1, true);
			pChr->EndlessHook(true, -1, true);
			if(pChr->m_Passive)
				pSelf->SendTuningParams(Victim, pChr->m_TuneZone);
		}
		else
		{
			pChr->m_Super = false;
			pChr->Teams()->SetForceCharacterTeam(Victim, pChr->m_TeamBeforeSuper);
			pChr->InfiniteJumps(false, -1, true);
			pChr->EndlessHook(false, -1, true);
			if(pChr->m_Passive)
				pSelf->SendTuningParams(Victim, pChr->m_TuneZone);
		}
	}
	...
	char aBuf[64];
	str_format(aBuf, sizeof(aBuf), "Invincibility for '%s': %s", pSelf->Server()->ClientName(Victim), pVictim->m_Invincible ? "ON" : "OFF");
	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "console", aBuf);
}
...
void CGameContext::ConForceFlagOwner(IConsole::IResult *pResult, void *pUserData)
{
	...
	((CGameControllerDDRace*)pSelf->m_pController)->ForceFlagOwner(Victim, pResult->GetInteger(0), true);
}
...
void CGameContext::ConReloadLanguages(IConsole::IResult *pResult, void *pUserData)
{
	...
	for (unsigned int i = 0; i < g_Localization.Languages().size(); i++)
		if (g_Localization.Languages()[i].m_Loaded)
			g_Localization.Load(g_Localization.Languages()[i].m_FileName.c_str(), true);
